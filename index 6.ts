...
const trustedDomains = [
  ...
  "bjanaesthesia.org",
  // Dermatology
  // jamanetwork.com already listed above — covers JAMA Dermatology too
  "aad.org",
...
function getResearchRole() {
  return `ROLE
You are an academic research assistant for physicians. You have web search capability — use it to find current, high-quality medical evidence.

SEARCH PRIORITY (strictly follow this hierarchy when searching):
- Tier 1 (ALWAYS prefer — use these first): NEJM, Lancet, JAMA, BMJ, Annals of Internal Medicine, Nature Medicine, Circulation, JACC
- Tier 2: AHA Journals, ACC, Science, Cell, Cochrane Library, CHEST, Gut, Blood, JCO
- Tier 3: Society guidelines (ATS, IDSA, ESMO, ACG, ESC, ADA, etc.), UpToDate
- Tier 4 (only if Tiers 1-3 insufficient): PubMed/PMC indexed specialty journals, MDPI journals
- NEVER cite: blogs, news sites, Wikipedia, patient-facing summaries, WebMD, Healthline, Mayo Clinic patient pages, Cleveland Clinic patient pages
ONLY use top-30 medical journals and PubMed-indexed sources. Prefer landmark RCTs, systematic reviews, meta-analyses, and current clinical guidelines.
Search for the most recent evidence available. Use multiple searches to be thorough — at least 3-4 searches per query.

CITATION RULES
- EVERY factual claim, statistic, trial result, or guideline recommendation MUST have an inline citation. No uncited claims.
- Aim for 8-12+ distinct sources per response. Use multiple searches to be thorough.
- Do NOT use numbered citations like [1], [2], [3].
- Do NOT include a references or bibliography section.
- Cite inline by linking to the source domain, e.g. ([nejm.org](https://nejm.org/...)). The system renders these as source pills automatically.

WRITING RULES
- Use blockquotes (>) sparingly — only for major guideline recommendations or landmark trial conclusions. Maximum 1-2 per response.
- Use professional manuscript-level language with subheadings.
...
- Use markdown tables for comparisons where possible.
- Paragraphs 2-3 sentences maximum.
- Do NOT ask the user for follow-up prompts.
...
- Answer directly — no lengthy background or context.
- Indicate when evidence is strong, mixed, or limited.
- Let the content dictate headings; avoid pre-set section names.
`;
}
// ==============================
// HELPER: Extract annotations from Responses API completed event
// ==============================
function extractAnnotationsAsCitations(responseData: any) {
  const citations: any[] = [];
  const seen = new Set();
  try {
    // Handle both direct response object and streaming wrapper { response: { ... } }
    const root = responseData?.response || responseData;
    ...
    for (const item of output) {
      ...
      for (const block of content) {
        ...
        for (const ann of annotations) {
          ...
          // Deduplicate by URL — frontend will match text positions itself
          if (seen.has(ann.url)) continue;
          seen.add(ann.url);
          let hostname = "";
          try { hostname = new URL(ann.url).hostname.replace("www.", ""); } catch {}
          citations.push({
            title: ann.title || "",
            url: ann.url,
            hostname,
...
// Old Tavily search functions removed — now using OpenAI web_search_preview
// ==============================
// SUPABASE RAG
// ==============================
...
Use professional medical terminology while remaining clear. If the image quality is poor or certain findings are uncertain, note this explicitly. Always provide your analysis with appropriate clinical context.`;
    ...
    // wantsRawSearch removed — web_search tool handles all search natively
    ...
    if (shouldUseClinical) {
      ...
      // ICD hints handled by LLM knowledge — no external lookup needed
      const icdHints = "";
      ...
      // Research mode — web_search tool handles retrieval natively
    ...
    // gpt-5-mini is faster for web_search modes; gpt-5.1 for reasoning-heavy modes
    let model;
    if (isReason || mode === "reason") model = "gpt-5.1";
    else if (mode === "search") model = "gpt-5-mini";
    ...
    // web_search requires at least "low" reasoning with GPT-5
    ...
    // Add web_search tool for research mode (non-clinical)
    // Journal quality enforced via system prompt; domain filtering may not be available on all models
    if (!shouldUseClinical) {
    ...
    if (!upstream.ok) {
      ...
      console.error(`❌ OpenAI API error: status=${upstream.status}, model=${model}, tools=${JSON.stringify(requestBody.tools)}, error=${errorText}`);
      console.error(`❌ Full request body keys: ${Object.keys(requestBody).join(', ')}`);
      return new Response(JSON.stringify({
    ...
    if (stream) {
              ...
              while ((sepIndex = buffer.indexOf("\n\n")) !== -1) {
                ...
                if (eventType === "response.output_text.delta") {
                  ...
                  controller.enqueue(encoder.encode(`data: ${JSON.stringify({
                    choices: [{ delta: { content: payload.delta } }]
                  })}\n\n`));
                } else if (eventType === "response.error") {
                  controller.enqueue(encoder.encode(`data: ${dataJson}\n\n`));
                } else if (eventType === "response.completed") {
                  // Extract url_citation annotations from completed response
                  try {
                    const completedData = JSON.parse(dataJson);
                    const annotations = extractAnnotationsAsCitations(completedData);
                    if (annotations.length > 0) {
                      controller.enqueue(encoder.encode(`data: ${JSON.stringify({ citations: annotations })}\n\n`));
                    }
                  } catch {}
      ...
      const annotations = extractAnnotationsAsCitations(data);
      const legacy: any = {
        id: data.id,
      ...
      if (annotations.length > 0) legacy.citations = annotations;
