...
TASK
Analyze the user's clinical vignette and produce an evidence-linked Bayesian differential diagnosis,
then the workup and management that follow from it. Always start with Differential Diagnosis.

DEPTH
Explain your thinking rather than asserting conclusions. A reader should be able to reconstruct why each
diagnosis sits where it does and what would move it. Prefer specific numbers, likelihood ratios, test
characteristics and named criteria over vague qualifiers. Do not pad with generic advice.

STRICT FORMAT — the differential block is machine-parsed, follow it exactly

## Differential Diagnosis

1. **[Condition name]** — *Likelihood XX–XX%*
   **Supporting**
   – [Finding, and why it raises probability — name the discriminating feature, not just the symptom]
   – [Lab, imaging or exam finding]
   – [Epidemiologic or risk-factor argument]
   **Against**
   – [Finding inconsistent with this diagnosis]
   – [Expected feature that is absent]
   **Discriminator** — [The single test or finding that would most efficiently confirm or exclude this]
   **Reasoning** — [3–5 sentences: the pretest probability you started from and why, how each key finding
   shifted it (give likelihood ratios or sensitivity/specificity when known), what makes this rank above or
   below its neighbours, and the specific result that would change the ranking.]

[Repeat for 4–6 diagnoses, ranked by likelihood, percentages summing to roughly 100%.]

FORMAT RULES
- Numbered line exactly: number, period, space, **bold condition**, em dash, *Likelihood XX–XX%*.
- Percentages are ranges, not single values, and every diagnosis has one.
- **Supporting**, **Against**, **Discriminator** and **Reasoning** each appear alone on a line, bolded.
  No other bolded labels inside a diagnosis block.
- Bullets use "– " (en dash + space). Never bold the bullet text itself.
- Never place a ## heading inside the Differential Diagnosis section.
- Every diagnosis carries at least one **Against** bullet, including the leading one.

## Clinical Reasoning
Two to three paragraphs: the problem representation in one sentence, the pivotal findings that most
constrained the differential, the base rates you assumed and why, and where you are genuinely uncertain.
Name any can't-miss diagnosis you are carrying at low probability and say what keeps it on the list.

## Next Steps
– **[Test or action]** — what you expect it to show, how each plausible result changes the differential
  above, and why it comes before the others.
– **[Test or action]** — same structure.
– **[Test or action]** — same structure.
– **Not indicated now** — one or two things a reader might reach for, and why they can wait.

## Management Considerations
– **[Intervention]** — dose or parameters, the trigger to start it, and the evidence behind it.
– **[Monitoring]** — specific parameters, thresholds, and the interval to recheck.
– **[Escalation]** — what would change the plan, stated as concrete criteria.

## Evidence
Two to four short paragraphs on the trials, guidelines or decision rules behind the recommendations above:
what each showed, in which population, and how directly it applies to this patient. Note where the evidence
is weak or extrapolated. No references list.
`;
...
TASK
Given the clinical scenario, provide evidence-based next diagnostic and management steps. Be specific and
actionable, and explain the reasoning behind each choice rather than only naming it.

${HOUSE_MARKDOWN_STYLE}

FORMAT
## Next Steps
– **[Test or action]** — expected findings, what each plausible result would rule in or out, test
  characteristics where known, and why it comes before the others.
– **[Test or action]** — same structure.
– **[Test or action]** — same structure.
– **Not indicated now** — one or two things a reader might reach for, and why they can wait.
...
– **Level of care** — the recommended setting and the reasoning that drives it, not just the label.
– **Escalation criteria** — specific findings that would move this patient up a level.
– **Discharge criteria** — what must be true before this patient can safely leave.`;
}
...
– **Severity/Acuity:** [How sick, anchored to specific vitals, labs, or trajectory]
– **Risk Stratification:** [Named score with the computed value and what that value predicts]
– **Need for Monitoring:** [What could deteriorate, how fast, and what would catch it]
– **Need for Interventions:** [What can only be delivered at this level of care]
– **Social/Safety:** [Concrete barriers to home management, not a generic statement]
...
– **One level down:** [What specifically makes a lower level unsafe here]
– **One level up:** [What is absent that would otherwise justify escalation]
...
1. **[Condition Name]** — *Likelihood XX–XX%*
   **Supporting**
   – [Clinical finding and why it raises probability]
   – [Lab/imaging finding]
   – [Epidemiologic factor]
   **Against**
   – [Finding inconsistent with this diagnosis]
   – [Missing expected feature]
   **Discriminator** — [The single most efficient test or finding to confirm or exclude this]
   **Reasoning** — [3–5 sentences on pretest probability, how each finding shifted it with likelihood
   ratios or test characteristics where known, why it ranks here, and what result would change that.]

2. **[Condition Name]** — *Likelihood XX–XX%*
   [Same structure]

[Continue for 4–6 diagnoses, ranked by likelihood, percentages summing to roughly 100%]

FORMAT RULES
- Numbered line exactly: number, period, space, **bold condition**, em dash, *Likelihood XX–XX%*.
- **Supporting**, **Against**, **Discriminator**, **Reasoning** each alone on a line, bolded. No other
  bolded labels inside a diagnosis block.
- Bullets use "– " (en dash + space); never bold bullet text.
- No ## heading inside the Differential Diagnosis section.
- Every diagnosis carries at least one **Against** bullet.

## Key Distinguishing Features
A table comparing the top three to four differentials across the features that actually separate them —
history, exam, labs, imaging. One row per feature, one column per diagnosis. Only include features that
discriminate; drop anything shared by all of them.

## Clinical Reasoning
Two to three paragraphs: the problem representation in one sentence, the pivotal findings, the base rates
you assumed and their source, how you weighted sensitivity and specificity of the key findings, and where
the estimate is fragile. State any can't-miss diagnosis carried at low probability and what keeps it there.`;
}
...
Use professional medical terminology while remaining clear. If the image quality is poor or certain findings are uncertain, note this explicitly. Always provide your analysis with appropriate clinical context.`;
    ...
    // Reasoning tokens are billed against max_output_tokens on the Responses API, so
    // the visible answer only gets what is left. The differential prompt now asks for a
    // Discriminator plus a 3-5 sentence Reasoning block on each of 4-6 diagnoses, on top
    // of Clinical Reasoning / Next Steps / Management / Evidence — that does not fit in
    // 5000, and a truncated answer drops the trailing sections and can cut a differential
    // entry mid-block (the client then discards that diagnosis entirely).
    const deepReasoningModes = new Set(["reason", "differential", "next-steps", "disposition", "dispo"]);
    const maxOutputTokens = (isReason || deepReasoningModes.has(mode)) ? 16000 : 5000;
