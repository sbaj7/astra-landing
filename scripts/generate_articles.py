...
def build_prompt(topic: str) -> str:
  ...
  return (
      "You are a clinical writer. Produce a JSON object (no markdown) that conforms to the following schema.\n"
      "The prose must be concise, evidence-based, and cite contemporary trials.\n\n"
      f"SCHEMA (FIELDS REQUIRED UNLESS MARKED OPTIONAL):\n{schema_description}\n\n"
      "RULES:\n"
      "- All arrays must have at least one item.\n"
      "- Icons must be selected from the enumerated options.\n"
      "- References must point to the canonical article page (journal or PubMed) using an https URL — do not return doi.org redirect links.\n"
      "- Include DOI information inside the detail string if helpful, but keep the url field as the full article page.\n"
  ...
  try:
    for raw_line in response.iter_lines(decode_unicode=False):
      ...
      if LOGGER.isEnabledFor(logging.DEBUG):
        LOGGER.debug("Raw SSE payload: %r", payload_str)
      try:
      ...
      if isinstance(payload_json, dict) and "citations" in payload_json:
        citations = payload_json.get("citations", []) or citations
        continue
