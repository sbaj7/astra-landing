...
export const renderMarkdown = (raw = '') => {
  ...
  const getDepth = (whitespace) => {
    const spaces = whitespace.replace(/\t/g, '    ');
    return Math.floor(spaces.length / 2); // 2 spaces per level
  };
  ...
  const openListAtDepth = (depth, type) => {
    // Ensure we have the right number of open lists
    while (listStack.length < depth) {
      out.push('<ul>');
      listStack.push({ type: 'ul', depth: listStack.length });
    }
    
    // If we need to change the type at current depth
    if (listStack.length === depth) {
      out.push(`<${type}>`);
      listStack.push({ type, depth });
    } else if (listStack[listStack.length - 1].type !== type) {
      // Replace the current list type
      out.push(`</${listStack.pop().type}>`);
      out.push(`<${type}>`);
      listStack.push({ type, depth });
    }
  ...
  while (i < lines.length) {
    ...
    if (numberMatch) {
      ...
      const depth = getDepth(whitespace);
      handleListItem(line, depth, content, 'ol');
      i++;
      continue;
    }
